...
The {{agent}} installation includes an `elastic-agent.reference.yml` file that describes all the settings available in a standalone configuration.
...
Each section of the file and available settings are also described in [Structure of a config file](/reference/ingestion-tools/fleet/structure-config-file.md).
