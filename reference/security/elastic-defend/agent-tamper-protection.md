...
# Prevent Elastic Agent uninstallation [agent-tamper-protection]
...
4. In the **Agent tamper protection** section, turn on the **Prevent agent tampering** setting.
    ...
    This makes the **Get uninstall command** link available, which you can follow to get the uninstall token and CLI command if you need to [uninstall an Agent](/reference/security/elastic-defend/uninstall-agent.md) on this policy.
...
::::{tip}
If you have many tamper-protected {{agent}} policies, you may want to [Provide multiple uninstall tokens](/reference/security/elastic-defend/uninstall-agent.md#multiple-uninstall-tokens) in a single command.
::::
