...
When you [configure inputs](/reference/fleet/elastic-agent-input-configuration.md) for standalone {{agents}}, the following values are supported for the input `type` parameter.
...
| `postgresql/metrics` | Periodically fetches metrics from [PostgreSQL](https://www.postgresql.org/) servers. | [PostgreSQL module](beats://reference/metricbeat/metricbeat-module-postgresql.md) ({{metricbeat}} docs) |
...
| Input | Description | Learn more |
| --- | --- | --- |
| `aws-cloudwatch` | Stores log filesfrom Amazon Elastic Compute Cloud(EC2), AWS CloudTrail, Route53, and other sources. | [AWS CloudWatch input](beats://reference/filebeat/filebeat-input-aws-cloudwatch.md) ({{filebeat}} docs) |
