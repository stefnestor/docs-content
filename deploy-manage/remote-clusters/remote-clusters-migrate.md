...
5. On the remote cluster, generate a cross-cluster API key that provides access to the indices you want to use for {{ccs}} or {{ccr}}. You can use the [Create cross-cluster API key]({{es-apis}}operation/operation-security-create-cross-cluster-api-key) API or [{{kib}}](../api-keys/elasticsearch-api-keys.md).
...
* Use the [Stop transforms]({{es-apis}}operation/operation-transform-stop-transform) API to stop any transforms.
* Use the [Close jobs]({{es-apis}}operation/operation-ml-close-job) API to close any anomaly detection jobs.
* Use the [Pause auto-follow pattern]({{es-apis}}operation/operation-ccr-pause-auto-follow-pattern) API to pause any auto-follow {{ccr}}.
* Use the [Pause follower]({{es-apis}}operation/operation-ccr-pause-follow) API to pause any manual {{ccr}} or existing indices that were created from the auto-follow pattern.
...
7. Use the [remote cluster info API]({{es-apis}}operation/operation-cluster-remote-info) to verify that the local cluster has successfully connected to the remote cluster:
...
* Use the [Start transform]({{es-apis}}operation/operation-transform-start-transform) API to start any transforms.
* Use the [Open jobs]({{es-apis}}operation/operation-ml-open-job) API to open any anomaly detection jobs.
* Use the [Resume follower]({{es-apis}}operation/operation-ccr-resume-follow) API to resume any auto-follow {{ccr}}.
* Use the [Resume auto-follow pattern]({{es-apis}}operation/operation-ccr-resume-auto-follow-pattern) API to resume any manual {{ccr}} or existing indices that were created from the auto-follow pattern.
...
7. Use the [remote cluster info API]({{es-apis}}operation/operation-cluster-remote-info) to verify that the local cluster has connected to the remote cluster. The response should have `"connected": true` and not have `"cluster_credentials": "::es_redacted::"`.
