You can configure and update dynamic settings on a running cluster using the [cluster update settings API]({{es-apis}}operation/operation-cluster-put-settings). You can also configure dynamic settings locally on an unstarted or shut down node using `elasticsearch.yml`.
...
In a self-managed cluster, you should use the [cluster update settings API]({{es-apis}}operation/operation-cluster-put-settings) to configure dynamic cluster settings, and only use `elasticsearch.yml` for static cluster settings and node settings. The API doesn’t require a restart and ensures a setting’s value is the same on all nodes.
