...
* **{{es}} nodes**: You should ensure that each {{es}} node in your cluster has at least 4 GB of RAM, across all [node roles](/deploy-manage/distributed-architecture/clusters-nodes-shards/node-roles.md) and [data tiers](/manage-data/lifecycle/data-tiers.md). Below this size, features such as [Logs and Monitoring](/deploy-manage/monitor/stack-monitoring/ece-ech-stack-monitoring.md), [Machine Learning](/explore-analyze/machine-learning.md), [Watcher](/explore-analyze/alerting/watcher.md), and [ILM](/manage-data/lifecycle/index-lifecycle-management.md) can cause service interruptions.
* **Clusters with dedicated master nodes**: For clusters with [dedicated master nodes](/deploy-manage/distributed-architecture/clusters-nodes-shards/node-roles.md#master-node-role), you should use at least 4 GB of RAM for each dedicated master node. This advice does not apply to 1 GB RAM tiebreaker nodes which auto-deploy in 2 zone setups. If a tiebreaker consistently experiences [High JVM Memory Pressure](/troubleshoot/elasticsearch/high-jvm-memory-pressure.md), you should expand to 3 zones of dedicated master nodes.
* **{{kib}} instances**: You should ensure that each {{kib}} instance in your cluster has at least 2 GB of RAM. Below this size, Platinum and Enterprise features such as [Security Detection Rules](/solutions/security/detect-and-alert.md), [Reporting](/explore-analyze/report-and-share.md), [Workflows](/explore-analyze/workflows.md), and [Agent Builder](/explore-analyze/ai-features/elastic-agent-builder.md) can cause service interruptions. {{kib}} instances in {{ech}} deployments created in May 2026 or later have 2 GB of RAM by default.
* **Integrations Server instances**: You should ensure that each Integrations Server instance in your cluster has at least 1 GB of RAM. Refer to [Fleet Server scalability](/reference/fleet/fleet-server-scalability.md#scaling-recommendations) for recommendations.
