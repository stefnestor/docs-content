...
To deploy more than one instance of {{kib}}, increase the `spec.count` value of the {{kib}} resource:
...
...
```
1. Set the `count` value to the desired amount of {{kib}} instances.

### {{kib}} Encryption keys [k8s-kibana-encryption-keys]

When deploying multiple {{kib}} instances, the instances must share a matching set of encryption keys. The following keys are automatically generated by the operator:
...
You can override these settings by providing your own encryption keys through secure settings. For a configuration example, refer to [{{kib}} secure settings](/deploy-manage/security/k8s-secure-settings.md#k8s-kibana-secure-settings).

For extra details about {{kib}} saved objects encryption, including ecryption key rotation configuration, refer to [Secure {{kib}} saved objects](/deploy-manage/security/secure-saved-objects.md).
