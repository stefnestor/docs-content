...
The cache can be expired manually with the [`clear-cache` API]({{es-apis}}operation/operation-indices-clear-cache):
...
It can also be enabled or disabled dynamically on an existing index with the [`update-settings`]({{es-apis}}operation/operation-indices-put-settings) API:
...
The size of the cache (in bytes) and the number of evictions can be viewed by index, with the [`indices-stats`]({{es-apis}}operation/operation-indices-stats) API:

```console
GET /_stats/request_cache?human
```

or by node with the [`nodes-stats`]({{es-apis}}operation/operation-nodes-stats) API:
