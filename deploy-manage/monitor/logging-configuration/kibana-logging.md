...
You can configure *[log level](#log-level)* and *appenders* for a specific logger. If a logger only has a *log level* configured, then the *appenders* configuration applied to the logger is inherited from the ancestor logger, up to the `root` logger.
...
##### Complete Example For Rewrite Appender [_complete_example_for_rewrite_appender]
