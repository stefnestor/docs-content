...
To help diagnose high CPU usage you can also use the {{es}} [nodes hot threads API]({{es-apis}}operation/operation-nodes-hot-threads), which identifies the threads on each node that have the highest CPU usage or that have been executing for a longer than normal period of time.
