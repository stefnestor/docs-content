...
You can submit an async search request using the [submit async search]({{es-apis}}operation/operation-async-search-submit) API. The [get async search]({{es-apis}}operation/operation-async-search-submit) API allows you to monitor the progress of an async search request and retrieve its results. An ongoing async search can be deleted through the [delete async search]({{es-apis}}operation/operation-async-search-submit) API.
