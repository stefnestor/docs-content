...
:::{note}
{applies_to}`{stack: "ga 9.2", serverless: "ga"}`
To view processes for OpenTelemetry hosts, you need to configure {{agent}} to send process metrics. Refer to [Process metrics](elastic-agent://reference/edot-collector/config/configure-metrics-collection.md#process-metrics) for more information.
:::
