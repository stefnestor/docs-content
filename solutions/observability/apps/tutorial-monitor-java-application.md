...
:::::::
1. Use the {{filebeat}} keystore to store [secure settings](asciidocalypse://docs/beats/docs/reference/ingestion-tools/beats-filebeat/keystore.md). Let’s store the Cloud ID in the keystore.
...
The good news is that all {{beats}} can process a log line before sending it to {{es}} by using [processors](asciidocalypse://docs/beats/docs/reference/ingestion-tools/beats-filebeat/filtering-enhancing-data.md). If the capabilities of these processors are not enough, you can always let {{es}} do the heavy lifting by using [an ingest node](../../../manage-data/ingest/transform-enrich/ingest-pipelines.md). This is what many modules in {{filebeat}} do. A module in {{filebeat}} is a way to parse a specific log file format for a particular software.
...
When the log level is `INFO`, it is logged with additional space at the end. You could use a [script processor](asciidocalypse://docs/beats/docs/reference/ingestion-tools/beats-filebeat/processor-script.md) and call `trim()`. However, it might be easier to fix our logging configuration to not always emit 5 characters, regardless of the log level length. You can still keep this when writing to standard out.
...
2. Now, on to the {{filebeat}} configuration. First, let’s remove all the processors, except the [add_host_metadata processor](asciidocalypse://docs/beats/docs/reference/ingestion-tools/beats-filebeat/add-host-metadata.md), to add some host information like the host name and operating system.
...
This requires you to download the standalone jar. You can find the link on the [official docs](asciidocalypse://docs/apm-agent-java/docs/reference/ingestion-tools/apm-agent-java/setup-attach-cli.md).
...
2. Instrument the application right at the start in our `main()` method.
    ...
    We did not configure any endpoint or API tokens yet. While the [documentation](asciidocalypse://docs/apm-agent-java/docs/reference/ingestion-tools/apm-agent-java/setup-attach-api.md#setup-attach-api-configuration) recommends using the `src/main/resources/elasticapm.properties` file, I prefer the use of environment variables, as this prevents either committing API tokens to your source or merging another repository. Mechanisms like [vault](https://www.vaultproject.io/) allow you to manage your secrets in such a way.
...
The agent can [trace methods](asciidocalypse://docs/apm-agent-java/docs/reference/ingestion-tools/apm-agent-java/config-core.md#config-trace-methods) based on their signature.
...
Once you have a bigger application with more code paths than our sample app, you can try to enable the [automatic profiling of inferred spans](asciidocalypse://docs/apm-agent-java/docs/reference/ingestion-tools/apm-agent-java/config-profiling.md#config-profiling-inferred-spans-enabled) by setting the following.
...
::::{important}
We have not covered the [Elastic APM OpenTracing bridge](asciidocalypse://docs/apm-agent-java/docs/reference/ingestion-tools/apm-agent-java/opentracing-bridge.md) or looked into the [additional metrics](asciidocalypse://docs/apm-agent-java/docs/reference/ingestion-tools/apm-agent-java/metrics.md) the agent provides, which allows us to take a look at things like garbage collection or the memory footprint of our application.
