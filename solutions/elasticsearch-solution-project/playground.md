...
* The {{es}} [Bulk API]({{es-apis}}operation/operation-bulk) for JSON documents
...
We've also provided some Jupyter notebooks to ingest sample data into {{es}}. Find these in the [elasticsearch-labs](https://github.com/elastic/elasticsearch-labs/blob/main/notebooks/ingestion-and-chunking) repository. These notebooks use the official {{es}} Python client.
...
Under **Indices**, you can edit which {{es}} indices are searched. This affects the underlying {{es}} query.
