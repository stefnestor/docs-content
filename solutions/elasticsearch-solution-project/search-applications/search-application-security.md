...
This guide describes best practices when taking the second approach. Specifically, we explain how to use search applications with frontend apps that make direct requests to the [Search Application Search API]({{es-apis}}operation/operation-search-application-search).
...
::::{tip} 
If `expiration` is not present, by default {{es}} API keys never expire. The API key can be invalidated using the [invalidate API key API]({{es-apis}}operation/operation-security-invalidate-api-key).
...
* [APIs](search-application-api.md)
    ...
    * [PUT Search Application API]({{es-apis}}operation/operation-search-application-put)
    * [Search Application Search API]({{es-apis}}operation/operation-search-application-search)
