...
To address the issue, observe the [Plugin flow rates]({{ls-apis}}#plugin-flow-rates) from the [Node Stats API]({{ls-apis}}), and identify which plugins have the highest `worker_utilization`. This will tell you which plugins are spending the most of the pipeline’s worker resources.
