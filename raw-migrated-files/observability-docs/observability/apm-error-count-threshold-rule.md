...
# Error count threshold rule [apm-error-count-threshold-rule]
...
## Filters and conditions [apm-error-count-threshold-rule-filters-conditions]

Filter the errors coming from your application to apply an Error count threshold rule to a specific service (`SERVICE`), environment (`ENVIRONMENT`) or error grouping key (`ERROR GROUPING KEY`). Alternatively, you can use a [KQL filter](../../../explore-analyze/query-filter/languages/kql.md) to limit the scope of the alert by toggling on the **Use KQL Filter** option.
...
:::{image} ../../../images/observability-apm-error-count-rule-filters-conditions.png
:alt: apm error count rule filters conditions
:::
...
## Rule schedule [_rule_schedule_2]
...
## Advanced options [_advanced_options_2]
...
### Action frequency [_action_frequency_2]
...
### Action variables [_action_variables]
...
:::{image} ../../../images/observability-apm-error-count-rule-action-variables.png
