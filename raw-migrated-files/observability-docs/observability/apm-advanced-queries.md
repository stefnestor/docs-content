...
## Querying in the Applications UI [apm-app-advanced-queries]

When querying in the Applications UI, you’re merely searching and selecting data from fields in {{es}} documents. Queries entered into the query bar are also added as parameters to the URL, so it’s easy to share a specific query or view with others.
...
Alternatively, you can query your APM documents in [**Discover**](../../../explore-analyze/discover.md). Querying documents in **Discover** works the same way as queries in the Applications UI, and **Discover** supports all of the example Applications UI queries shown on this page.
...
::::{tip}
Starting in v7.6, you can view ten samples per bucket in the Applications UI, instead of just one.
::::
...
You can now explore the data until you find a specific transaction that you’re interested in. Copy that transaction’s `transaction.id` and paste it into the Applications UI to view the data in the context of the Applications UI:
