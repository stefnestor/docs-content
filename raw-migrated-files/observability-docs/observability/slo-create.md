---
navigation_title: "Create an SLO"
---

# Create a service-level objective (SLO) [slo-create]
...
* [Custom KQL](../../../solutions/observability/incident-management/create-an-slo.md#custom-kql-sli) — create an SLI based on raw logs coming from your services.
* [Custom metric](../../../solutions/observability/incident-management/create-an-slo.md#custom-metric-sli) — create an SLI to define custom equations from metric fields in your indices.
* [Timeslice metric](../../../solutions/observability/incident-management/create-an-slo.md#timeslice-metric-sli) — create an SLI based on a custom equation that uses multiple aggregations.
* [Histogram metric](../../../solutions/observability/incident-management/create-an-slo.md#histogram-metric-sli) — create an SLI based on histogram metrics.
* [APM latency and APM availability](../../../solutions/observability/incident-management/create-an-slo.md#apm-latency-and-availability-sli) — create an SLI based on services using application performance monitoring (APM).
* [Synthetics availability](../../../solutions/observability/incident-management/create-an-slo.md#synthetics-availability-sli) — create an SLI based on the availability of your synthetic monitors.


### Custom KQL [custom-kql-sli]
...
### Custom metric [custom-metric-sli]
...
### Timeslice metric [timeslice-metric-sli]
...
### Histogram metric [histogram-metric-sli]
...
### APM latency and APM availability [apm-latency-and-availability-sli]


#### APM latency [apm-latency-sli]
...
#### APM availability [apm-availability-sli]

Create an indicator based on the availability of your instrumented services. Availability is determined by calculating the percentage of successful transactions (`event.outcome : "success"`) out of the total number of successful and failed transactions—unknown outcomes are excluded.
...
* **Monitor name** — The name of one or more [synthetic monitors](../../../solutions/observability/apps/configure-synthetics-projects.md#synthetics-configuration-monitor-name).
...
* **Tags** — One or more [tags](../../../solutions/observability/apps/configure-synthetics-projects.md#synthetics-configuration-monitor-tags) assigned to synthetic monitors.
...
|     |     |
| --- | --- |
| **Occurrences** | Uses the number of good events and the number of total events to compute the SLO.<br>**Example:** You have a 30 day rolling SLO with a 95% target, and, over the past 30 days, there were 1,355,700 total events. The error budget is `100-95 = 5%`, or about 66,785 bad events are tolerated before violating the SLO.<br>If you had 1,300,000 good events over the same period, the observed value is `Good Events / Total Events = 0.95891421 => 95.89%`. |
| **Timeslices** | Breaks the overall time window into smaller slices of a defined duration, and uses the number of good slices over the number of total slices to compute the SLO.<br>**Timeslice target (%)** - Individual timeslices target that determines if the slice is good or bad.**Timeslice window (in minutes)** - The size of the timeslice window size.<br>**Example:** A 30 day rolling SLO defined with five minute slices has a total of `30*24*12 = 8640` slices.If the SLO target is 98%, we have a `100-98 = 2%` error budget or `8640 * 0.02 = 172` bad slices available before we violate the SLO. |


### Set your time window [slo-time-window]
...
## Create an SLO burn rate alert rule [slo-alert-checkbox]
