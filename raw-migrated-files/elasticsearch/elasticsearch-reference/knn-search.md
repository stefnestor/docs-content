---
navigation_title: "kNN search"
---

# k-nearest neighbor (kNN) search [knn-search]
...
* To run a kNN search, you must be able to convert your data into meaningful vector values. You can [create these vectors using a natural language processing (NLP) model in {{es}}](../../../explore-analyze/machine-learning/nlp/ml-nlp-text-emb-vector-search-example.md), or generate them outside {{es}}. Vectors can be added to documents as [`dense_vector`](https://www.elastic.co/guide/en/elasticsearch/reference/current/dense-vector.html) field values. Queries are represented as vectors with the same dimension.
