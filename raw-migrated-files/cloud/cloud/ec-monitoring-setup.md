...
When issues come up that you need to troubleshoot, you’ll frequently start with the same queries to determine which rabbit hole to investigate further, such as [`_cluster/health`]({{es-apis}}operation/operation-cluster-health) to determine overall deployment health.
