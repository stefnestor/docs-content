...
- **Correlate signals across data sources.** Use [{{es}} search actions](/explore-analyze/workflows/steps/elasticsearch.md) to query metrics, logs, and traces in the same workflow, then combine results with [`if` steps](/explore-analyze/workflows/steps/if.md) to decide on next actions.
...
- **Analyze signals with AI.** Use [AI steps](/explore-analyze/workflows/steps/ai-steps.md) to have an agent interpret anomaly patterns or summarize a multi-signal investigation before the workflow takes action.
