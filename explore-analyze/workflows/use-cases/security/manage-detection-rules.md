...
{applies_to}`stack: ga 9.5+` {applies_to}`serverless: ga` To enable or disable rules from a workflow, prefer the named [`security.enableRule`](/explore-analyze/workflows/steps/detection-rules.md#security-enablerule) and [`security.disableRule`](/explore-analyze/workflows/steps/detection-rules.md#security-disablerule) steps over a generic `kibana.request` call to the detection engine `_bulk_action` API.

## What you can automate [workflows-rule-ops-patterns]

The following patterns combine [scheduled triggers](/explore-analyze/workflows/triggers/scheduled-triggers.md) with named [Detection rules action steps](/explore-analyze/workflows/steps/detection-rules.md) and [{{kib}} request actions](/explore-analyze/workflows/steps/kibana.md#kibana-request) to drive rule-operations work:
...
- {applies_to}`stack: ga 9.5+` {applies_to}`serverless: ga` **Enable or disable rules by ID or query.** Use `security.enableRule` / `security.disableRule` to change rule state with schema-validated inputs and partial-failure reporting, instead of hand-writing a bulk-action request.
...
- [Detection rules action steps](/explore-analyze/workflows/steps/detection-rules.md): Enable or disable detection rules with `security.enableRule` and `security.disableRule`.
