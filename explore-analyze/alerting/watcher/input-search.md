...
| `request.indices_options.allow_no_indices` | no | `true` | Whether to allow (1) wildcard index expressions that match no indices and (2) requests where the final resolved set is empty. Refer to [allow_no_indices](elasticsearch://reference/elasticsearch/rest-apis/api-conventions.md#api-multi-index)                                                                                    for more information. |
