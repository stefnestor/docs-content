...
* Query Elasticsearch APIs other than the search API. For example, you might want to load data from the [nodes stats]({{es-apis}}operation/operation-nodes-stats), [cluster health]({{es-apis}}operation/operation-cluster-health) or [cluster state]({{es-apis}}operation/operation-cluster-state) APIs.
...
To load the data from other Elasticsearch APIs, specify the API endpoint as the `path` attribute. Use the `params` attribute to specify query string parameters. For example, the following `http` input calls the [cluster stats]({{es-apis}}operation/operation-cluster-stats) API and enables the `human` attribute:
