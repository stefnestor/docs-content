...
* Transforms might have more configuration options provided by the APIs than the options available in {{kib}}. For all the transform configuration options, refer to the [API documentation]({{es-apis}}group/endpoint-transform).
...
If you prefer to spread out the impact on your cluster (at the cost of a slower transform), you can throttle the rate at which it performs search and index requests. Set the `docs_per_second` limit when you [create]({{es-apis}}operation/operation-transform-put-transform) or [update]({{es-apis}}operation/operation-transform-update-transform) your transform. If you want to calculate the current rate, use the following information from the [get transform stats API]({{es-apis}}operation/operation-transform-get-transform-stats):
