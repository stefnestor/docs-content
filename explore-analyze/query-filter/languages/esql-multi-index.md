...
It can be helpful to know the particular index from which each row is sourced. To get this information, use the [`METADATA`](elasticsearch://reference/query-languages/esql/esql-metadata-fields.md) option on the [`FROM`](elasticsearch://reference/query-languages/esql/commands/source-commands.md#esql-from) command.
