...
The {{graph-features}} provide a simple, yet powerful [graph exploration API]({{es-apis}}group/endpoint-graph), and an interactive graph visualization app for {{kib}}. Both work out of the box with existing {{es}} indices—you don’t need to store any additional data to use these features.
