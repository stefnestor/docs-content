...
To configure the ML inference pipeline, we need the index to have an existing field mapping so we can choose which field to analyze. This can be done via the [index mapping API]({{es-apis}}operation/operation-indices-put-mapping) in the Kibana Dev Tools or simply through a cURL command:
