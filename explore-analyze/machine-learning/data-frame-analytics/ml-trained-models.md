...
1. Given a model *name*, find the model *ID*. You can use `curl` to call the [get trained model API]({{es-apis}}operation/operation-ml-get-trained-models) to list all models with their IDs.
...
2. Using `curl` from the command line, again use the [get trained models API]({{es-apis}}operation/operation-ml-get-trained-models) to export the entire model definition and save it to a JSON file.
...
3. Import the saved model using `curl` to upload the JSON file to the [created trained model API]({{es-apis}}operation/operation-ml-put-trained-model). When you specify the URL, you can also set the model ID to something new using the last path part of the URL.
...
* Models exported from the [get trained models API]({{es-apis}}operation/operation-ml-get-trained-models) are limited in size by the [http.max_content_length](elasticsearch://reference/elasticsearch/configuration-reference/networking-settings.md) global configuration value in {{es}}. The default value is `100mb` and may need to be increased depending on the size of model being exported.
