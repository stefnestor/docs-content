...
If you see a large number of script cache evictions and a rising number of compilations in [node stats]({{es-apis}}operation/operation-nodes-stats), your cache might be too small.
...
To update existing data, use this pipeline to [reindex]({{es-apis}}operation/operation-reindex) any documents from `my_test_scores` to a new index named `my_test_scores_2`.
