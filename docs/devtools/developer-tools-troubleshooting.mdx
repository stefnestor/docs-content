...
Use the [**Exists API**](/api-reference/search/indices-exists) to check whether a data stream, index, or
alias exists:
...
If the field is aggregatable, you can use <DocLink id="serverlessElasticsearchExploreYourDataAggregations" text="aggregations" />
...
Use the [**Explain API**](((ref))/search-explain.html) to find out why a specific document matches or doesn’t match a query:
...
You can update dynamic index settings with the [**Update index settings API**](/api-reference/search/indices-put-settings-1). Changing dynamic index settings for a data stream {/*  <DocLink id="enElasticsearchReferenceModifyDataStreams" section="change-a-dynamic-index-setting-for-a-data-stream">Changing dynamic index settings for a data stream</DocLink> */} requires changing the index template used by the data stream.
