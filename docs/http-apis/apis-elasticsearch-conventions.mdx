...
description: The ((es)) REST APIs have conventions for headers, request bodies, and units of data.
...
There are common conventions for specifying units of data, such as bytes, meters, and days in ((es)) APIs. 

### Byte size units

If you specify the size of data, you must also specify its unit.
...
If you specify a distance and without a unit, the default unit is typically `meters`.
...
| `m` or `meters` | meters |
| `cm` or `centimeters` | centimeters |
| `mm` or `millimeters` | millimeters |
| `NM`, `nmi`, or `nauticalmiles` | nautical miles |

For example, set the `distance` parameter in a geo-distance query to `"2mi"` for 2 miles.

### Time units

If you specify a duration of time, you must also specify its unit.
