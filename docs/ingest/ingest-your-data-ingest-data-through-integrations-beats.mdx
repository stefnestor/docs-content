...
When you use ((beats)) to export data to an ((es-serverless)) project, the Beat needs to use API-key based authentication.
For an example, refer to [Grant access using API keys](https://www.elastic.co/guide/en/beats/filebeat/current/beats-api-keys.html) in the Filebeat documentation.
