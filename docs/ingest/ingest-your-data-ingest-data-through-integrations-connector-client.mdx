...
<DocTabs hasBorder>
  <DocTab name="Source">
    **Run from source**
    ...
    - Clone or fork the repository locally with the following command:
      ...
      ```
    - Open the [`config.yml` configuration file](https://github.com/elastic/connectors/blob/main/config.yml) in your editor of choice.
  ...
  <DocTab name="Docker">
    **Run with Docker**
    ...
    This is a good option if you don't want to worry about your local Python environment.
    The [individual connector references](#available-connectors) contain concrete instructions on how to run your connector with Docker.
    Refer to [`DOCKER.md`](https://github.com/elastic/connectors/blob/main/docs/DOCKER.md) in the repo for a more detailed guide.

    At a high-level, these are the steps you'll follow:

    - Create a Docker network.
    - Create a directory to be mounted into the Docker image.
    - Download the sample configuration file ([`config.yml`](https://github.com/elastic/connectors/blob/main/config.yml)) into that directory.
    - Update the config file with your specific values:
      - `host` (your ((es)) endpoint)
      - `api_key` (((es)) API key)
      - `connector_id`
      - `service_type`
