...
Prepend any ((kib)) API endpoint with `kbn:` and send the request through <DocBadge><DocIcon size="s" type="unlink" title="missing link"/> missing link</DocBadge>{/*  <DocLink id="enKibanaConsoleKibana">Dev Tools > Console</DocLink> */}.
For example:
