...
An API key is a private, unique identifier for authentication and authorization. You'll need a private API key to securely connect to your project. You can use an existing key, or create a new one.

1. On the **Get started with Elasticsearch** page, scroll down to **Prepare an API Key** and click **New**.

2. From the **Create an API key** panel, enter a name for your key and its expiration. Then click **Create API key**.

After you created your API key, it is displayed with its ID, name, expiration date, and its encoded version. Save this information somewhere safe as it is displayed only once and it is not stored anywhere in the project.

#### Using an API key

Pass your `encoded` API key in an `Authorization` header using the `ApiKey` scheme:
...
<DocTabs> 
  ...
  <DocTab name="Ruby">
    ...
    # Ingest the data via the bulk API:
