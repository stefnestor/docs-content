...
2. From the **Create an API key** panel, enter a name for your key and its expiration. 

After you created your API key, save it somewhere safe as it is displayed only once and it is not stored anywhere in the project.
...
<DocTabs>
  <DocTab name="curl">
    ...
    ```bash
    curl -X GET '${ES_URL}/books/_search' -H 'Content-Type: application/json' -d'
    {
        "query": {
            "match": { "_all": "snow" }
        }
