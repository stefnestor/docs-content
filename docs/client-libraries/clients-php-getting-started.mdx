...
To get API keys or the Cloud URL for a project, see 
<DocLink id="serverlessElasticsearchGetStarted" />.
...
If you try to get the document ID, you will get a `ClientResponseException` with 
a `404 Not Found` message, as follows:
...
printf("Operation result: %s\n", $response['result']); # You get "deleted" as a result.
```
...
This operation is possible only if the associated API key has 
`delete_index`or 
[manage index privilege](https://www.elastic.co/guide/en/elasticsearch/reference/current/security-privileges.html#privileges-list-indices)
