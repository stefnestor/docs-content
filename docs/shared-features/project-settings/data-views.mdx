...
If you collected data using one of the ingest options, uploaded a file, or added sample data, you get a ((data-source)) for free, and can start exploring your data.
...
{/* <DocImage size="m" url="images/discover-find-data-view.png" alt="How to set the ((data-source)) in Discover" /> */}
 ...
 1. Open {/***Lens** or*/}**Discover** then open the data view menu.
    ![How to set the ((data-source)) in Discover](images/discover-find-data-view.png)
    Alternatively, go to the **((data-views-app))** app in **((project-settings)) > ((manage-app))** or **((project-settings))**.
...
You can manage your data view in the **((data-views-app))** app.
...
<DocCallOut title="Temporary ((data-sources)) are not available in the ((data-views-app)) app." />
...
<DocCallOut color="warning" title="Deleting a ((data-source)) breaks all visualizations, saved searches, and other saved objects that reference the data view." />

1. Go to the **((data-views-app))** app in **((project-settings))** or **((project-settings)) > ((manage-app))**.
