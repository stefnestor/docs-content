...
Ultimately, when configuring `groups` for a job, think in terms of how you might wish to partition data in a query at a future date… then include those in the config. Because Rollup Search allows any order or combination of the grouped fields, you only need to decide if a field is useful for aggregating later, and how you might wish to use it (terms, histogram, and so on).

## Calendar versus fixed time intervals [rollup-understanding-group-intervals]
...
| Unit | Calendar | Fixed |
| --- | --- | --- |
| millisecond | NA | `1ms`, `10ms`, and so on |
| second | NA | `1s`, `10s`, and so on |
| minute | `1m` | `2m`, `10m`, and so on |
| hour | `1h` | `2h`, `10h`, and so on |
| day | `1d` | `2d`, `10d`, and so on |
| week | `1w` | NA |
| month | `1M` | NA |
| quarter | `1q` | NA |
| year | `1y` | NA |
