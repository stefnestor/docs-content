...
You can manage source indices just like regular {{es}} indices using the [document]({{es-apis}}group/endpoint-document) and [index]({{es-apis}}group/endpoint-indices) APIs.
...
After adding enrich data to your source indices, use the [create enrich policy API]({{es-apis}}operation/operation-enrich-put-policy) or [Index Management in {{kib}}](/manage-data/ingest/transform-enrich/data-enrichment.md#manage-enrich-policies) to create an enrich policy.
...
Once the enrich policy is created, you need to execute it using the [execute enrich policy API]({{es-apis}}operation/operation-enrich-execute-policy) or [Index Management in {{kib}}](/manage-data/data-store/perform-index-operations.md#manage-indices) to create an [enrich index](data-enrichment.md#enrich-index).

:::{image} /manage-data/images/elasticsearch-reference-enrich-policy-index.svg
:alt: enrich policy index
:::

The *enrich index* contains documents from the policy’s source indices. Enrich indices always begin with `.enrich-*`, are read-only, and are [force merged]({{es-apis}}operation/operation-indices-forcemerge).
...
Define an [enrich processor](elasticsearch://reference/enrich-processor/enrich-processor.md) and add it to an ingest pipeline using the [create or update pipeline API]({{es-apis}}operation/operation-ingest-put-pipeline).
...
Before implementing the pipeline in production, we recommend indexing a few test documents first and verifying enrich data was added correctly using the [get API]({{es-apis}}operation/operation-get).


## Update an enrich index [update-enrich-data]

Once created, you cannot update or index documents to an enrich index. Instead, update your source indices and [execute]({{es-apis}}operation/operation-enrich-execute-policy) the enrich policy again. This creates a new enrich index from your updated source indices. The previous enrich index will be deleted with a delayed maintenance job that executes by default every 15 minutes.

If wanted, you can [reindex]({{es-apis}}operation/operation-reindex) or [update]({{es-apis}}operation/operation-update-by-query) any already ingested documents using your ingest pipeline.
...
1. Create and [execute]({{es-apis}}operation/operation-enrich-execute-policy) a new enrich policy.
2. Replace the previous enrich policy with the new enrich policy in any in-use enrich processors or {{esql}} queries.
3. Use the [delete enrich policy]({{es-apis}}operation/operation-enrich-delete-policy) API or [Index Management in {{kib}}](/manage-data/ingest/transform-enrich/data-enrichment.md#manage-enrich-policies) to delete the previous enrich policy.


## Enrich components [ingest-enrich-components]

The enrich coordinator is a component that manages and performs the searches required to enrich documents on each ingest node. It combines searches from all enrich processors in all pipelines into bulk [multi-searches]({{es-apis}}operation/operation-msearch).
...
:   Maximum number of [force merge]({{es-apis}}operation/operation-indices-forcemerge) attempts allowed on an enrich index. Defaults to `3`.
